...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/scientists_pca_concat_title.png}
    \caption{Mapa naukowców WMIi w~przestrzeni PCA (2D) dla konkatenacji
    embeddingów tytułów z~czterech modeli: MiniLM-L6, SPECTER,
    MPNet-Base, Multilingual-MiniLM.
    Kolor punktu odpowiada liczbie publikacji danego naukowca.}
    \label{fig:pca_concat}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/coauthor_evaluation.png}
    \caption{Rozkład podobieństwa cosinusowego dla par współautorskich
    i~losowych par naukowców -- SPECTER (lewy panel) i~MiniLM-L6 (prawy panel).
    Przerywane linie oznaczają średnie grupowe.}
    \label{fig:coauthor_hist}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/model_comparison.png}
    \caption{Porównanie SPECTER i~MiniLM-L6: średnie podobieństwo kosinusowe
    (lewy), rozmiar efektu $d$ Cohena (środkowy) i~różnica bezwzględna
    między grupami (prawy). Przerywane linie~-- progi małego, średniego
    i~dużego efektu według skali Cohena.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{pictures/model_correlation.png}
    \caption{Korelacja podobieństw kosinusowych SPECTER i~MiniLM-L6
    dla wszystkich $\sim$6\,600 par autorów (Pearson $r = 0{,}729$).
    Czerwona przerywana linia~-- idealna zgodność modeli.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{pictures/jaccard_heatmap_title.png}
    \caption{Macierz średniego podobieństwa Jaccarda (top-10 sąsiadów)
    między sześcioma modelami dla embeddingów tytułów.
    Ciemniejszy kolor oznacza wyższą zgodność rankingów sąsiadów.}
...
\input{CHAPTERS/centroid_stability_section}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/heatmap_gorecki_abstract.png}
    \caption{Heatmapa stabilności centroidu prof.~UAM dr~hab.~Tomasza Góreckiego
    (naukowiec interdyscyplinarny, 114~prac, embeddingi abstraktów).
    Czerwone obszary przy małym $k$ świadczą o~dużej wrażliwości modeli ogólnych
    na~dobór próbki; modele SPECTER i~BGE-Small zachowują wysoką stabilność.}
    \label{fig:heatmap_gorecki}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/heatmap_rucinski_abstract.png}
    \caption{Heatmapa stabilności centroidu prof.~dr~hab.~Andrzeja Rucińskiego
    (kombinatoryka, 199~prac, embeddingi abstraktów).
    Jednorodna specjalizacja przekłada się na~szybszą stabilizację centroidu
    nawet dla modeli ogólnych -- kontrast z~profilem Góreckiego.}
    \label{fig:heatmap_rucinski}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/correlation_metrics.png}
    \caption{Korelacja Pearsona i~Spearmana między czterema metrykami profilu
    naukowca: liczbą publikacji, zgodnością Jaccarda, średnim podobieństwem
    sąsiadów i~średnią odległością sąsiadów.
    Liczba prac nie koreluje z~Jaccardem ($r=0{,}04$),
    ale koreluje umiarkowanie ze~średnim podobieństwem sąsiadów ($r=0{,}26$).}
