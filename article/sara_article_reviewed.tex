...
%   TODO : tutaj potrzeba dodać jeszcze akapit przejścia, rozwiązania klasyczne, manualne wyszukiwanie (google scholar i inne). Dać więcej motywacji i kontekstu dla potrzeby wizualizacji i grupowania autorów i ich publikacji

%   TODO : tutaj bym po pierwsze zamienił kolejnośc pytań, te drugie jest zdecydowanie wazniejsze badawczo. Druga sprawa to język, w artykułach narracja jest bardziej płaska i oficjalna. Po trzecie brakuje wykazania motywacji dlaczego wogóle potrzeba redukować do d=2
...
%   TODO : w artykule trzeba to prezentować jako algorytm/metodę/protokół. W każdym razie nie system informatyczny bo jako taki nie ma żadnej wartości naukowej. Wartościowe naukowo mogą być tylko metody i narzędzia zaimplementowane w systemie a nie samo oprogramowanie.
%   TODO : jeśli to miałoby iść gdzie kolwiek dalej niż na WMI, to albo trzeba przemilczeć zasięg danych, albo rozbudować na większą grupę.
...
%   Badania obejmowały, ogólnie to trzeba by przerobić bardziej w kierunku zakresu i celu badań
System obejmuje:
\begin{enumerate}
...
% ============================================================
%   TODO : Ogólnie druga sekcja artykułu to tak zwane 'materials and methods' trzeba sie trzymać tej narracji i nazwnictwa
\section{Dane i~embeddingi}
\label{sec:data}
% ============================================================

%   TODO : jako materials można to opisać bardziej ogólnie jako Dataset preparation
...
%   TODO: tutaj napewno warto dopisać kryteria wykluczania danych


%   TODO : to trzeba bardziej wyjaśnić, dodać dosłownie zdanie kontekstu, ze to jest dobre źródło dobrze opisanych tematycznie artykułów. Warto też wspomnieć jaką role ma pełnić w badaniach
...
%   TODO : to jest tak naprawdę kolejny etap przygotowania danych
...
%   TODO : to tez trzeba opisać bardziej jako decyzje badawcza, ze rozważono takie a takie modele, odwołąć się do tabelki, i napisać że wybrano ten model jako najlepiej sprawujący się na zbiorze danych ArXiv
...
%   TODO : na ten moment ta sekcja tutaj nie ma sensu. odwołanie do tego lematu jest dobrym pomysłem bo jest to nośny temat, ale raczej później w analizie wyników.
...
% ============================================================
%   TODO : jako 3 sekcja powinno być Proposed method
%   tutaj proponuję aby była to jedna wspólna sekcja podzielona na dwie podsekcje poświęcone redukcji i reprezentacji. Konieczne jest też uwzględnienie tych dwóch obszarów we wstępie tak zeby ich istnienie wynikało z ogólnej narracji i nie zaskakiwało czytalnieka dopiero tutaj
...
%   ogólnie każda z tych podsekcji powinna mieć następującą strukturę (możliwe ze bez potrzeby wyróżniania podsekcji):
%   zajawka i motywacja problemu, potem jego formalna deficnicja, następnie metody redukcji i ich podział, metodologia porównania, zaproponowanie Waszego composite score, 

Porównano osiem metod DR, podzielonych na dwie grupy ze~względu na
charakter zachowywanych relacji.

%  to każdej metody trzeba dodać cytownie na jakiś źróðłowy paper ją wprowadzający
...
%  tutaj proponował bym narracje ze są to niezbędne elementy dla budowy Waszego compisite score
...
%  wyniki proponuję przenieśc od osobnej głównej sekcji Results and discussion
%   czasem też odziela się results i discussion do osobnych sekcji. Cały ten podział ma na celu rozdzielene zaproponowanych nowych metod (proposed approach) od wyników i szczegółów ewaluacji (results) oraz często subiektywej interpretacji wyników przez autora (discussion). 
%  Przykładowo wybór liczby klastrów to jest już informacja o samym eksperyencie i nie stanowi części zproponowanego przez was rozwiązania, wiec powinna wylądować w sekcji wynikowej opisująć setup ekspetymnetu
...
%   to już jest bardziej discussion bo mamy tutaj intepretację wyników, można to połączyć z results, ale optymalnie jest wydzielić, o ile takich wniosków jest wystaraczjąco dużo zeby mogłby być samodzielną sekcja. Ponadto w discussion ponownie można odwołać się do wyników z innych badań, zeby je porównać
...
%   struktura jak wyżej, tutaj jest znacznie bliżej jej zachowania
...
%   to dalej jest element proposed approach, chodzi o to zeby tutaj przedstawić kompletne Wasze rozwiązanie problemu profilu naukowca, a dostrajanie jest postulowaną ważną jego częścią
...
%   ale to już jest bardziej do results, bo opisuje rzeczywistą realizację eksperymentu
...
%  i wtedy po tym naturalnie idą wyniki dostrajania do results
...
% to natoiast jest kluczowym i koncowym elementem do omówienia w proposed approach
...
% ============================================================
%   to bedzie sekcja Results, tutaj też pewnie trzeba bedzie utrzymać dwusekcyjny charakter narracji z podziełem na reduckje i na profilowanie
\section{Ewaluacja}
\label{sec:eval}
% ============================================================

%  widzę że w obecnej wersji results i dyskusja/interpretacja jest mocno przeplatana, narazie proponuję to tak zostawić, ewentualnie rozdzielenie zrobimy w następnym etapie jak już całośc narracji się bardziej ustabilizuje
